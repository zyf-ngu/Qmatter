...
class VectorRetriever:
    def __init__(self, embed_model_path: str = r"E:\Qagent\models\allMiniLML6v2"):
        if not embed_model_path:
            raise ValueError("必须提供本地嵌入模型路径")
        try:
            self.model = SentenceTransformer(embed_model_path, local_files_only=True)
        except Exception:
            word_embedding_model = models.Transformer(embed_model_path, local_files_only=True)
...
class Reranker:
    def __init__(self, model_path: str = None):
        if not model_path:
            raise ValueError("必须提供本地重排序模型路径")
